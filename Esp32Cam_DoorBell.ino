...
void sendDataToThingSpeak() {
  ThingSpeak.setField(1, String(lastPhotoTimestamp));
  ThingSpeak.setField(2, String(photoCount));
...
void loop() {
  if (sendPhoto) {
    ...
    // Update ThingSpeak after sending photo
    lastPhotoTimestamp = millis() / 1000;  // Get the current timestamp in seconds
  ...
  if (digitalRead(BUTTON) == LOW) {
    ...
    // Update ThingSpeak after button press
    lastPhotoTimestamp = millis() / 1000;  // Get the current timestamp in seconds
    photoCount++;  // Increment photo count
    sendDataToThingSpeak();
  }
